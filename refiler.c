/* 
Refiler is based on https://github.com/nelhage/reptyr
See README an COPYING for more information 
...
#ifndef __linux__
#error refiler is currently Linux-only.
#endif
...
void usage(char *me) {
    fprintf(stderr, "Usage: %s PID_PROCESS FILE_DESCRIPTOR NEW_FILE\n", me);
}
...
int main(int argc, char **argv) {
    ...
    if (do_attach) {
        ...
        int err;
	//char *my_buffer="/tmp/myfile2";
        if ((err = attach_child(child, argv[3],atoi(argv[2])))) {
